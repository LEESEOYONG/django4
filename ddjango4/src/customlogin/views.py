...
def signin(request):
    ...
    #POST 방식요청
        ...
        #username 과 비밀번호가 일치하는 User 객체 한개를 추출하는 함수
        #만약, username이나 비밀번호가 틀린경우, None 값을 반환함
        u = authenticate(username=id,password=pw)
        #유저가 데이터베이스에 있는지 확인
        if u:
            #로그인처리
            #login(클라이언트,User객체) : 해당 요청을 한 클라이언트가 User 객체
            ...
            #비로그인상태의 클라이언트는 request.user에  None 값이 저당되어있음
            login(request,u)
        ...
        #유저가 존재하지 않는 경우
            ...
            return render(request,'cl/signin.html',
                          {'form':form, 'error':'아이디나 비밀번호가올바르지않다.'})
            
        
#로그아웃
def signout(request):
    #logout(클라이언트정보) : 해당 클라이언트의 User정보를 지워주는 함수
    #request.user 에 None 값으로 변경됨
    #만약 비로그인 상태의 클라이언트가 접근하더라도 에러가 발생하지 않음
    logout(request)
    #reverse('그룹이름:별칭',args=(추가변수의 값,))
    #그룹이름과 별칭이 일치하는 URL 주소를 문자열형태로 반환해주는 함수
    #만약 추가매개변수가 있는 뷰함수인 경우, args 매개변수를 통해
    #매개변수에 넘져줄 값을 저장할 수 있음(튜플)
    #ex)detail뷰함수reverse('',args=(q_id,))
